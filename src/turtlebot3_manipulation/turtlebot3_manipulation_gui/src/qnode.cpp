...
void QNode::updateRobotState()
{
  ...
  if (!jointValues.empty() && !jointValues2.empty()) {
    ...
    // Handle the case where the vectors are empty
    // This could be by logging an error, throwing an exception, etc.
    ROS_ERROR("jointValues or jointValues2 is empty");
    return;
  }
